...
# Jalankan dengan:
# python -m streamlit run dashboard_inovasi.py
...
def load_data(uploaded_file: Optional[io.BytesIO] = None) -> pd.DataFrame:
    """Memuat data dari file Excel yang diunggah atau default."""
    ...
    after = len(df)
    if before != after:
        st.info(f"🧹 Hapus duplikat: {before - after} baris dihapus. Tersisa {after} baris.")
    else:
    ...
    # Konversi tanggal
    for col in ['Tanggal Input', 'Tanggal Penerapan', 'Tanggal Pengembangan']:
    ...
    for c in ['Jenis', 'Bentuk Inovasi', 'Admin OPD', 'Kategori Admin OPD', 'Urusan Utama', 'Asta Cipta', 'Daerah']:
        if c in df.columns:
            df[c] = df[c].astype(str).replace(['nan', 'NaN', 'None'], np.nan)

    # Tambahkan grup Admin OPD
...
# =====================================================
# 📥 SIDEBAR: FILTER & UNGGAH DATA
# =====================================================
with st.sidebar:
    st.header("📂 Sumber Data & Filter")
    uploaded_file = st.file_uploader("Unggah file Excel (.xlsx) atau biarkan kosong", type=['xlsx'])
    df = load_data(uploaded_file if uploaded_file is not None else None)

    st.markdown("---")
    st.subheader("🔎 Filter umum")

    min_kematangan = st.number_input("Kematangan minimal", min_value=0, value=0, step=1)

    # Filter Jenis
    jenis_selected = ['All']
    if 'Jenis' in df.columns:
        jenis_options = sorted(df['Jenis'].dropna().unique().astype(str).tolist())
        jenis_selected = st.multiselect("Pilih Jenis", ['All'] + jenis_options, default=['All'])

    # Filter OPD
    opd_selected = ['All']
    if 'Admin OPD Grouped' in df.columns:
        opd_options = sorted(df['Admin OPD Grouped'].dropna().unique().tolist())
        opd_selected = st.multiselect("Pilih OPD", ['All'] + opd_options, default=['All'])
    ...
    with st.expander("⚙️ Filter tambahan"):
        ...
        if 'Kategori Admin OPD' in df.columns:
            ...
            kategori_selected = st.multiselect("Kategori Admin OPD", ['All'] + kategori_options, default=['All'])

        urusan_selected = ['All']
        if 'Urusan Utama' in df.columns:
            urusan_options = sorted(df['Urusan Utama'].dropna().unique().astype(str).tolist())
            urusan_selected = st.multiselect("Urusan Utama", ['All'] + urusan_options, default=['All'])

st.info("💡 Tips: Klik grafik untuk interaksi. Gunakan filter di atas untuk memperbarui seluruh tampilan.")
...
# 🔍 FILTER DATA
# =====================================================
def apply_filters(df, min_kematangan, jenis_selected, opd_selected, kategori_selected, urusan_selected):
    filtered = df.copy()
    if 'Kematangan' in filtered.columns:
        filtered = filtered[filtered['Kematangan'] >= min_kematangan]
    if 'Jenis' in filtered.columns and 'All' not in jenis_selected:
        filtered = filtered[filtered['Jenis'].isin(jenis_selected)]
    if 'Admin OPD Grouped' in filtered.columns and 'All' not in opd_selected:
        filtered = filtered[filtered['Admin OPD Grouped'].isin(opd_selected)]
    if 'Kategori Admin OPD' in filtered.columns and 'All' not in kategori_selected:
        filtered = filtered[filtered['Kategori Admin OPD'].isin(kategori_selected)]
    if 'Urusan Utama' in filtered.columns and 'All' not in urusan_selected:
        filtered = filtered[filtered['Urusan Utama'].isin(urusan_selected)]
    return filtered


df_filtered = apply_filters(df, min_kematangan, jenis_selected, opd_selected, kategori_selected, urusan_selected)
...
if 'Bentuk Inovasi' in df_filtered.columns:
    ...
    # Hanya satu tabel di bawah grafik
    st.markdown("### 📋 Tabel Bentuk Inovasi")
...
def load_map_data():
    ...
    df_map.columns = df_map.columns.str.lower()  # seragamkan nama kolom
    return df_map

map_jatim = load_map_data()

# ======================================================
# 2️⃣ CEK KOLOM LATITUDE & LONGITUDE
# ======================================================
...
if lat_col and lon_col:
    ...
    # ======================================================
    # 3️⃣ IDENTIFIKASI NAMA DAERAH (OFFLINE)
    # ======================================================
    st.info("🔍 Mengidentifikasi nama daerah berdasarkan koordinat (offline cache)...")
    ...
    # ======================================================
    # 4️⃣ TAMPILKAN SEMUA DAERAH LANGSUNG
    # ======================================================
    daerah_tersedia = sorted(df_geo["Daerah"].dropna().unique())

    st.write("🗺️ **Sebaran Semua Inovasi di Jawa Timur (tanpa perlu memilih daerah)**")

    # Gunakan rata-rata seluruh titik sebagai pusat peta
    center_lat, center_lon = df_geo[lat_col].mean(), df_geo[lon_col].mean()
    m = folium.Map(location=[center_lat, center_lon], zoom_start=8, tiles="cartodb positron")

    # Warna berbeda tiap daerah
    color_list = px.colors.qualitative.Set3
    warna_daerah = {daerah: color_list[i % len(color_list)] for i, daerah in enumerate(daerah_tersedia)}

    # Tambahkan marker per inovasi
    for _, row in df_geo.iterrows():
        ...
        folium.CircleMarker(
            ...
            radius=5,
            color=warna_daerah.get(daerah, "gray"),
            fill=True,
            fill_color=warna_daerah.get(daerah, "gray"),
            popup=popup_html
        ).add_to(m)
    ...
    <div style="position: fixed; 
        ...
        <b>Legenda Warna Daerah</b><br>
    """
    for daerah, color in warna_daerah.items():
        legend_html += f"<span style='color:{color}'>⬤</span> {daerah}<br>"
    legend_html += "</div>"
    ...
    # ======================================================
    # 6️⃣ RANGKUMAN METRIK & DISTRIBUSI
    # ======================================================
    col1, col2 = st.columns(2)
    col1.metric("Jumlah Inovasi", len(df_selected))
    if "Kematangan" in df_selected.columns:
        col2.metric("Rata-rata Kematangan", f"{df_selected['Kematangan'].mean():.2f}")

    # Distribusi Jenis Inovasi (Pie Chart)
    if "Jenis" in df_selected.columns and not df_selected["Jenis"].isna().all():
        ...
        fig_jenis = px.pie(
            jenis_counts, names="Jenis", values="Jumlah",
            title=f"Distribusi Jenis Inovasi di {selected_daerah}",
            hole=0.4,
    ...
    # ======================================================
    # 7️⃣ BAR CHART JUMLAH INOVASI PER DAERAH
    # ======================================================
    st.write("📊 **Jumlah Inovasi per Daerah di Jawa Timur**")
    if "Daerah" in df_geo.columns and not df_geo["Daerah"].dropna().empty:
        inovasi_per_daerah = df_geo.groupby("Daerah")["Judul Inovasi"].count().reset_index()
        inovasi_per_daerah = inovasi_per_daerah.rename(columns={"Judul Inovasi": "Jumlah Inovasi"})
        fig_bar = px.bar(
            inovasi_per_daerah.sort_values("Jumlah Inovasi", ascending=False),
            x="Daerah", y="Jumlah Inovasi", text="Jumlah Inovasi",
            color="Jumlah Inovasi", color_continuous_scale="Blues",
            title="Jumlah Inovasi per Daerah"
        )
        fig_bar.update_layout(xaxis_tickangle=-45)
        st.plotly_chart(fig_bar, use_container_width=True)
    else:
        st.warning("⚠️ Data 'Daerah' kosong, bar chart tidak dapat ditampilkan.")

    # ======================================================
    # 8️⃣ TABEL DETAIL INOVASI
    # ======================================================
    ...
    # ======================================================
    # 9️⃣ ANALISIS MAYORITAS TOPIK INOVASI PER DAERAH
    # ======================================================
    st.write("🧠 **Analisis Topik Dominan di Daerah Ini**")

    # Pilih kolom teks yang akan dianalisis
    text_col = None
    for c in df_selected.columns:
        if "deskripsi" in c.lower():
            text_col = c
            break
    if text_col is None and "Judul Inovasi" in df_selected.columns:
        text_col = "Judul Inovasi"

    if text_col:
        # Gabungkan semua teks inovasi jadi satu
        all_text = " ".join(str(t).lower() for t in df_selected[text_col].dropna())

        # Bersihkan teks dari kata umum
        stopwords = set([
            "inovasi","untuk","dan","yang","dalam","pada","dengan","sebagai",
            "dari","ini","bagi","guna","melalui","atau","pemerintah","daerah","program"
        ])
        ...
        if word_freq:
            ...
            fig_words = px.bar(
                ...
                title=f"Kata yang Paling Sering Muncul dalam Inovasi di {selected_daerah}"
            )
            fig_words.update_layout(xaxis_tickangle=-45)
            st.plotly_chart(fig_words, use_container_width=True)

            # Word Cloud
            st.write("☁️ **Word Cloud Topik Inovasi:**")
            ...
            st.info("Tidak ada kata yang cukup dominan untuk dianalisis.")
    else:
        st.warning("Tidak ditemukan kolom teks seperti 'Judul Inovasi' atau 'Deskripsi' untuk analisis topik.")

else:
    st.warning("⚠️ Kolom latitude dan longitude tidak ditemukan di data.")
...
if 'df_filtered' in locals() and isinstance(df_filtered, pd.DataFrame) and not df_filtered.empty:
    ...
    df_compare = pd.DataFrame({
        ...
        'Daerah': ['Jakarta', 'Bandung', 'Surabaya']  # tambahkan kolom Daerah untuk visualisasi
    })
...
# ==========================================================
# Visualisasi Jumlah Inovasi per Daerah
# ==========================================================
st.subheader("📊 Jumlah Inovasi per Daerah")
if 'Daerah' in df_compare.columns:
    inovasi_per_daerah = df_compare.groupby('Daerah')['Judul Inovasi'].count().reset_index()
    inovasi_per_daerah = inovasi_per_daerah.rename(columns={'Judul Inovasi': 'Jumlah Inovasi'})

    st.dataframe(inovasi_per_daerah.sort_values(by='Jumlah Inovasi', ascending=False))

    fig = px.bar(
        inovasi_per_daerah.sort_values(by='Jumlah Inovasi', ascending=False),
        x='Daerah',
        y='Jumlah Inovasi',
        text='Jumlah Inovasi',
        color='Jumlah Inovasi',
        color_continuous_scale='Blues',
        title='Jumlah Inovasi per Daerah'
    )
    fig.update_layout(xaxis_title='Daerah', yaxis_title='Jumlah Inovasi', xaxis_tickangle=-45)
    st.plotly_chart(fig, use_container_width=True)
else:
    st.warning("⚠️ Kolom 'Daerah' tidak ditemukan di dataset. Tidak dapat membuat visualisasi.")

# ==========================================================
# Pilihan Inovasi untuk Kolaborasi
# ==========================================================
...
max_comb = st.slider(
    ...
    help="Gunakan slider ini untuk membatasi berapa banyak kombinasi inovasi yang akan dianalisis oleh AI."
)
